...
- The recording HUD must stay click-through (`configureHUDPanelForPassthrough`). It is informational, but a borderless panel still swallows clicks across its whole frame on every screen, over whatever the user was reaching for mid-dictation. Non-activating is not enough. Remove this only if the HUD gains a real control.
